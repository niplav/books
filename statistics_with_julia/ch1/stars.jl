...
blurImg=[boxBlur(gImg, x, y, 5) for x in 1:rows, y in 1:cols]

yOriginal, xOriginal=argmax(gImg).I
yBoxBlur, xBoxBlur=argmax(blurImg).I

p1=heatmap(gImg, c=:Grey, yflip=true)
...
plot(p1, p2, size=(800,400), ration=:equal, xlims=(0,cols), ylims=(0,rows), colorbar_entry=false, border=:none, legend=:none)