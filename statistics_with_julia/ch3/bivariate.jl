...
density_integral=sum(z)*delta^2
println("2-dimensional Riemann sum over density: ", density_integral)

prob_b=sum([sum([f(x,y)*delta for y in x:delta:1])*delta for x in grid])
println("2-dimensional Riemann sum to evaluate probability: ", prob_b)
